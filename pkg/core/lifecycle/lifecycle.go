...
// This function waits on a SIGINT or SIGTERM signal and shuts down the fetcher server gracefully.
func TerminateHandler(logger log.Logger, server core.ShutDowner) {
	...
	if err != nil {
		logger.Error(fmt.Sprintf("failed to shutdown server: %s", err.Error()))
	}
}
